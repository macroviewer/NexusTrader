...
- we use [uvloop](https://github.com/MagicStack/uvloop) to speed up the event loop, which is 2-4x faster than the default event loop.
- we use [picows](https://github.com/tarasko/picows) which is a cython implementat websocket framework and can achieve the similar performance of C++ boost.beast. Other python frameworks using the websockets like `websockets` and `aiohttp` are slower than `picows`.
- we use msgspec to serialize and deserialize the data, which is faster than orjson, ujson, json, etc. All the data classes are defined as the `msgspec.Struct` and `msgspec.Struct` is faster than `dataclass`.
- All the order are placed in the `asyncio.Queue`
- The core (MessageBus, Clock, etc.) are implemented in the Rust and the python code is generated by the [nautilius](https://github.com/nautilius/nautilius) framework.
