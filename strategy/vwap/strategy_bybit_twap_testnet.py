            ...
            if not order_id:
                ...
                if side == OrderSide.BUY:
                    ...
                    if (rs := (current_time - start)/1000) > make_time:
                        self.log.debug(
                            f"Symbol: {symbol} {rs}/{duration}s -> take order"
                        )
                        price = book.ask
                        on_bid = False
                    else:
                        self.log.debug(
                            f"Symbol: {symbol} {rs}/{duration}s -> make order"
                        )
                        price = book.bid
                        on_bid = True
                ...
                remaining_interval = max(1, remaining_seconds / interval)
                size = float((amount - pos)) / remaining_interval
                self.log.debug(
